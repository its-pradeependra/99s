...
export default function EventView() {
    ...
    const prevSlide = () => {
        slideNumber === 0
            ? setSlideNumber(event.imageUrls.length - 1)
            : setSlideNumber(slideNumber - 1)
    }

    // Next Image  
    const nextSlide = () => {
        slideNumber + 1 === event.imageUrls.length
            ? setSlideNumber(0)
            : setSlideNumber(slideNumber + 1)
    }
    ...
    return (
        <main className=" max-w-4xl mx-auto space-y-8 bg-slate-50 py-20 md:py-20 lg:py-48 bg-slate-50 pt-[42%] md:pt-[8%]">
            ...
            {event && !loading && !error && (
                <div>
                    <h1 className=" text-center mb-10 text-3xl lg:text-4xl font-semibold leading-9 text-gray-800 pb-4">{event.heading} </h1>
                    <div className="grid grid-cols-1 md:grid-cols-3 mx-auto">
                        {
                            event.imageUrls.map((url, index) => (
                                <div key={url} onClick={() => handleOpenModal(index)} className='py-8 px-4 mx-auto max-w-screen-xl cursor-pointer'>
                                    <img className="w-60 h-48 rounded-xl" src={url} alt={url} /></div>
                            ))
                        }
                    </div>

                    {openModal && (
                        <div className="slideWrap grid fixed top-0 bottom-0 left-0 right-0 z-50 bg-black bg-opacity-80 items-center justify-center w-full h-full p-10">
                            {/* Cross Button */}
                            <button onClick={handleCloseModal} className="fixed cursor-pointer opacity-70 text-white z-50 hover:opacity-100 top-10 right-10">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-10">
                                    <path strokeLinecap="round" strokeLinejoin="round" d="M6 18 18 6M6 6l12 12" />
                                </svg>
                            </button>
                            {/* Previous Button */}
                            <button onClick={prevSlide} className="fixed cursor-pointer opacity-60 text-white z-50 hover:opacity-100 top-[50%] left-10 transform -translate-y-1/2">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="h-6 w-6">
                                    <path strokeLinecap="round" strokeLinejoin="round" d="M10.5 19.5L3 12m0 0l7.5-7.5M3 12h18" />
                                </svg>
                            </button>
                            {/* Next Button */}
                            <button onClick={nextSlide} className="fixed cursor-pointer opacity-60 text-white z-50 hover:opacity-100 top-[50%] right-10 transform -translate-y-1/2">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="h-6 w-6">
                            ...
                            <img
                                src={event.imageUrls[slideNumber]}
                                alt="Gallery Image"
                    ...
                    {/* Share Button */}
                    <div className='fixed top-[25%] bg-black text-white ease-in-out duration-700 hover:text-black hover:bg-white right-[3%] z-10 border rounded-full w-12 h-12 flex justify-center items-center bg-slate-100 cursor-pointer'>
                        <FaShare
                            className=''
                            onClick={() => {
